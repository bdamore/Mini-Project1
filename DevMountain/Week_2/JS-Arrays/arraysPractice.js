...
var reversedLooper = function(letters) {
  var inverseLetters = letters.reverse();
  for (i = 0; i < inverseLetters.length; i++) {
    alert(inverseLetters[i]);
  }
}
...
var divider = function(nums, evens, odds) {
  ...
  for (i = 0; i < nums.length; i++) {
    if (nums[i] % 2 == 0) {
      evens.push(nums[i]); //didnt take into account for 0
    } else {
      odds.push(nums[i]);
    }
  }
...
function finder (numbers) {
   var x = getRandomArbitrary(); debugger
...
var compareArrays = function(array1, array2) {
  if (array1.length > array2.length) {
    ...
    return "Neither array is longer than the other"
  }
  else {
    return array2;
  }
